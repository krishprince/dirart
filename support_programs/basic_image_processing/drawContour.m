...
if ~isempty(contourV)
    for i = 1:length(contourV)
        segment = contourV{i};
        if ~isempty(segment)
            hContour = [hContour, line(segment(:,1), segment(:,2), 'color', 'blue', 'linewidth', .5, 'hittest', 'off', 'erasemode', 'normal', 'userdata', i, 'ButtonDownFcn', 'drawContour(''contourClicked'')', 'parent', hAxis)];
        end
    end
